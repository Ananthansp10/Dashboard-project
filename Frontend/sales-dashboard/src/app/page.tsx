...
const DASHBOARD_LABEL_DEFAULT_USAGE: Record<string, string[]> = {
  totalRevenue: ["dashboard_summary"],
  totalOrders: ["dashboard_summary"],
  totalCustomers: ["dashboard_summary"],
...
export default function DashboardPage() {
      ...
      try {
        ...
        if (labelRes.status === "fulfilled" && Array.isArray(labelRes.value?.labels)) {
          ...
          const usageMap = labelRes.value.labels.reduce((acc: Record<string, string[]>, item: { key: string; usedIn?: string[] }) => {
            ...
            acc[item.key] =
              apiUsedIn.length ? apiUsedIn : DASHBOARD_LABEL_DEFAULT_USAGE[item.key] || [];
            return acc;
          }, { ...DASHBOARD_LABEL_DEFAULT_USAGE });
          setLabelUsage(usageMap);
        } else {
          setLabelUsage(DASHBOARD_LABEL_DEFAULT_USAGE);
        }
        ...
        console.error("Failed to load labels or table data", error);
      }
    };
  ...
  const handleEditLabel = (key: string) => {
    const usages = labelUsage[key] ?? DASHBOARD_LABEL_DEFAULT_USAGE[key] ?? [];
        ...
        <section className="grid grid-cols-2 gap-6 mb-10 max-lg:grid-cols-1">
          <div className="bg-white rounded-xl shadow-sm p-6 relative">
            <div className="flex justify-between items-center mb-6">
              <LabelWithEdit label={labels.revenueByMonth} onEdit={() => handleEditLabel("revenueByMonth")} />
            </div>
            <BaseChart
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 relative">
            <div className="flex justify-between items-center mb-6">
              <LabelWithEdit label={labels.ordersByCategory} onEdit={() => handleEditLabel("ordersByCategory")} />
            </div>
            <BaseChart
